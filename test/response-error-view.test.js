...
describe('<response-error-view>', () => {
  async function basicFixture() {
    return await fixture(`<response-error-view></response-error-view>`);
  }
  ...
  describe('_messageChanged()', () => {
    ...
    [
      ...
      it(`Opens page for ${item[0]}`, () => {
        element._messageChanged(item[0]);
        assert.equal(element.detailsPage, item[1]);
      });
    });
  });
});
